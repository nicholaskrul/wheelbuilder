...
# --- 1. APP CONFIGURATION ---
st.set_page_config(page_title="ProWheel Lab v9.8", layout="wide", page_icon="🚲")
...
st.title("🚲 ProWheel Lab v9.8: Dynamic Spec Manager")
st.markdown("---")
...
with tabs[1]:
    ...
    try:
        ...
        side = st.radio("Stage to Wheel side:", ["Front", "Rear"], horizontal=True)
        if st.button("Apply and Stage"):
            ...
            st.success(f"{side} staged at {holes}h!")
...
with tabs[2]:
    ...
    with st.form("lib_form_v98", clear_on_submit=True):
        b, m = st.text_input("Brand"), st.text_input("Model")
        w = st.number_input("Weight (g)", 0.0, step=0.1)
        if l_type == "Rims":
            e, h = st.number_input("ERD (mm)", 601.0, step=0.1), st.number_input("Hole Count", 28, step=2)
            ...
            ol, orr = st.number_input("L-Offset", 30.0), st.number_input("R-Offset", 30.0)
...
with tabs[4]:
    ...
    if not df_builds.empty:
        ...
        # Calculation Logic: (2 rims) + F-hub + R-hub + (Mass x VARIABLE Qty)
        ...
        if not pd.isna(total_w):
            ...
            wc3.metric("Total Set Weight", "Missing Data")
