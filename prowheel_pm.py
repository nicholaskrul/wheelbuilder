...
CACHE_DATA_TTL = 3600  
WORKSHOP_CAPTION = "Workshop Command Center | Intelligent Autofill Enabled"
...
def render_admin_pipeline():
    ...
    st.title("🚲 Wheelbuilder Lab v18.80")
    ...
    # =========================================================================
    # --- TAB 3: UPGRADED BUILD REGISTRY WITH LIVE AUTOFILL MATCHING ---
    # =========================================================================
    with tabs[2]:
        ...
        with c_f:
            ...
            # --- LIVE FRONT AUTOFILL INTERCEPTOR ---
            f_combo = f"{fr_rim} | {fr_hub}"
            f_match = df_recipes[df_recipes['label'] == f_combo] if not df_recipes.empty else pd.DataFrame()
            if fr_rim != "None" and fr_hub != "None" and not f_match.empty:
                default_fl = safe_float(f_match.iloc[0].get('len_l', 0.0))
                default_fr = safe_float(f_match.iloc[0].get('len_r', 0.0))
                st.caption("✨ Proven front recipe found! Autopopulating lengths.")
            else:
                default_fl, default_fr = 0.0, 0.0
                
            fl_len = st.number_input("Left Spoke Length (mm)", value=default_fl, step=0.1, key=f"fl_input_{fr_rim}_{fr_hub}")
            fr_len = st.number_input("Right Spoke Length (mm)", value=default_fr, step=0.1, key=f"fr_input_{fr_rim}_{fr_hub}")
        ...
        with c_r:
            ...
            # --- LIVE REAR AUTOFILL INTERCEPTOR ---
            r_combo = f"{rr_rim} | {rr_hub}"
            r_match = df_recipes[df_recipes['label'] == r_combo] if not df_recipes.empty else pd.DataFrame()
            if rr_rim != "None" and rr_hub != "None" and not r_match.empty:
                default_rl = safe_float(r_match.iloc[0].get('len_l', 0.0))
                default_rr = safe_float(r_match.iloc[0].get('len_r', 0.0))
                st.caption("✨ Proven rear recipe found! Autopopulating lengths.")
            else:
                default_rl, default_rr = 0.0, 0.0
                
            rl_len = st.number_input("Left Spoke Length (mm) ", value=default_rl, step=0.1, key=f"rl_input_{rr_rim}_{rr_hub}")
            rr_len = st.number_input("Right Spoke Length (mm) ", value=default_rr, step=0.1, key=f"rr_input_{rr_rim}_{rr_hub}")
