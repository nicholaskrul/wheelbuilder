...
# --- 1. APP CONFIGURATION ---
st.set_page_config(page_title="Wheelbuilder Lab v11.5", layout="wide", page_icon="🚲")
...
def calculate_precision_spoke(erd, fd, os, holes, crosses, is_sp, sp_offset, hole_diam=2.4, round_mode="None"):
    """
    Refined in v11.5: Adjusts for straightpull tangential path discrepancies in crossed lacing.
    ...
    if not is_sp:
        ...
        # For crossed lacing, we calculate the path length considering 3D tangential exit
...
with tabs[2]:
    ...
    with st.form("lib_form_v11_5", clear_on_submit=True):
    ...
    try:
        ...
        with st.expander("➕ Add New Inventory Item"):
            with st.form("add_inv_item"):
                model_sel = st.selectbox("Select Model", df_spk_lib['brand'] + " " + df_spk_lib['model'])
        ...
        if not df_inv.empty:
            ...
            with st.form("inventory_update_v11_5"):
                updated_data = []
                ...
                if st.form_submit_button("💾 Update Stock Levels"):
                    ...
                    st.rerun()
    except Exception as e: st.info("Ensure 'spoke_inventory' exists in your sheet.")
...
with tabs[4]:
    ...
    try:
        ...
        with st.form("build_form_v11_5"):
            ...
            st.info(f"💡 **Staged Lengths:** F: {st.session_state.f_l}/{st.session_state.f_r} | R: {st.session_state.r_l}/{st.session_state.r_r}")
        ...
        if not df_builds.empty:
            ...
            def get_safe_w(sheet_name, part_name):
                ...
                try:
                    ...
                    match = df[(df['brand'] + " " + df['model']) == str(part_name)]
                    if not match.empty:
