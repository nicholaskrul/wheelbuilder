...
# --- 1. APP CONFIGURATION ---
st.set_page_config(page_title="Wheelbuilder Lab v14.3", layout="wide", page_icon="🚲")
...
# --- 5. MAIN UI ---
st.title("🚲 Wheelbuilder Lab v14.3")
st.caption("Integrated Component Management & Precision Workshop")
        ...
        for _, row in f_df.sort_values('id', ascending=False).iterrows():
            with st.expander(f"🛠️ {row.get('customer')} — {row.get('status')} ({row.get('date')})"):
                ...
                with c3:
                    ...
                    new_stat = st.selectbox("Status", ["Order Received", "Parts Received", "Building", "Complete"], key=f"st_{row['id']}", 
...
with tabs[2]:
    ...
    with st.form("reg_v14_3"):
        ...
        if build_type in ["Full Wheelset", "Front Only"]:
            with cf:
                st.subheader("Front")
                fr, fh = st.text_input("Rim", value=st.session_state.build_stage['f_rim']), st.text_input("Hub", value=st.session_state.build_stage['f_hub'])
        ...
        if build_type in ["Full Wheelset", "Rear Only"]:
            with cr:
                st.subheader("Rear")
                rr, rh = st.text_input("Rim", value=st.session_state.build_stage['r_rim']), st.text_input("Hub", value=st.session_state.build_stage['r_hub'])
        ...
        payload.update({"spoke": sc1.selectbox("Spoke", df_spk['label'] if not df_spk.empty else ["Standard"]),
                        "nipple": sc2.selectbox("Nipple", df_nip['label'] if not df_nip.empty else ["Standard"]),
...
# --- TAB 5: LIBRARY ---
with tabs[4]:
    st.header("📦 Component Library Management")
    with st.expander("➕ Add New Component to Library", expanded=False):
        cat = st.radio("Category", ["Rim", "Hub", "Spoke", "Nipple"], horizontal=True)
        with st.form("lib_add"):
            name = st.text_input(f"{cat} Name")
            c1, c2 = st.columns(2)
            data = {}
            if cat == "Rim":
                data = {"rim": name, "erd": c1.number_input("ERD (mm)", step=0.1), "holes": c2.number_input("Hole Count", step=1, value=28), "weight": st.number_input("Weight (g)", step=0.1)}
            elif cat == "Hub":
                data = {"hub": name, "fd_l": c1.number_input("FD Left", step=0.1), "fd_r": c2.number_input("FD Right", step=0.1), 
                ...
                data = {cat.lower(): name, "weight": st.number_input("Weight per unit (g)", format="%.3f", step=0.001)}
            
            if st.form_submit_button(f"Save {cat} to Airtable"):
                if not name: st.error("Name is required.")
                else:
                    base.table(f"{cat.lower()}s").create(data)
